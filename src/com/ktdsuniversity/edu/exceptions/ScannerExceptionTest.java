...
public class ScannerExceptionTest {
	...
	public static void main (String[] args) {
		 ...
		 * }
		 * catch(try에서 발생하는 예외클래스 작성. 파라미터처럼 ==> NumberFormatException exception) {
		 * 		예외를 다루는 후처리 코드
		 * 
		 * }
		 */
		...
		try {
			...
			// java.util.InputMismatchException 발생됨
			numberString = keyboard.nextInt() + ""; // 숫자 형태가 아닌 문자들을 작성하면 예외 발생
			
			System.out.println("숫자를 입력하셨군요?");
		}
		catch(InputMismatchException exception) {
			System.out.println("예외가 발생했습니다");
			System.out.println("아마도 숫자가 아닌 문자를 입력했을 수 있습니다");
		}
